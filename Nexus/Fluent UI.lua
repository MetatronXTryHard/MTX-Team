...
if isfile(d) then
    local _, _ =
        pcall(
        function()
            getgenv().blacklist = e:JSONDecode(readfile(d))
        end
    )
end
local function _()
...
local function c()
    local a = tick()
    local _ = getgenv().lastExecutionTime or 0
    return (a - _) >= b
end
task.spawn(
    function()
        while wait() do
            if getgenv().blacklist.Blacklisted then
                _()
                wait(1)
                return
            end
...
if getgenv().Key == "CyWzYWELREXR2y4mKtYZkQYhDVDZvFvG" or getgenv().Key == "WyWzYWEQRERRwyDmKtMZkQYhwVDZLFvB" then
    premium = "premium"
end
task.spawn(
    function()
...
task.spawn(
    function()
        while true do
            ...
            local a = premium
            if a ~= _ then
                getgenv().blacklist.Blacklisted = true
                writefile(d, e:JSONEncode(getgenv().blacklist))
                _ = a
                wait(1)
                return
            end
        ...
        return function(n)
            ...
            o.CloseButton =
                ...
                function()
                    p.Window:Dialog {
                        Title = "Close",
                        Content = "Are you sure you want to uninject?",
